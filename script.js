...
function renderModerationQueue() {
    ...
    if (moderationQueue.length === 0) { 
        queueContainer.innerHTML = `<p class="no-ideas">${translations[currentLang].queue_empty}</p>`; 
        return; 
    }
    
    moderationQueue.forEach(item => {
        const title = currentLang === 'ru' ? item.titleRu : (item.titleKy || item.titleRu);
        const desc = currentLang === 'ru' ? item.descRu : (item.descKy || item.descRu);
        const langBadge = item.titleRu ? 'RU' : 'KY';
        ...
        div.innerHTML = `
            <div class="mod-item-content">
                <div class="mod-item-header">
                    <h4>${title}</h4>
                    <span class="mod-lang-badge">${langBadge}</span>
                </div>
                <p class="mod-item-author"><strong>${translations[currentLang].author_txt}:</strong> ${item.author}</p>
                ${desc ? `<p class="mod-item-desc"><strong>${translations[currentLang].desc_txt}:</strong> ${desc}</p>` : ''}
            </div>
            <div class="mod-item-actions">
                <button class="btn btn-approve" onclick="approveIdea(${item.id})">
                    ${translations[currentLang].btn_approve_act}
                </button>
                <button class="btn btn-reject" onclick="rejectIdea(${item.id})">
                    ${translations[currentLang].btn_reject_act}
                </button>
            </div>
        `;
